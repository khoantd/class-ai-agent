...
[OntoSight](https://www.npmjs.com/package/royalsolution-ontosight) visualizes [CodeGraph](https://github.com/colbymchenry/codegraph) call subgraphs in an interactive browser UI. **class-ai-agent** installs usage rules across all four agent trees; invocation via **`royalsolution-ontosight`** (`npx royalsolution-ontosight` or global install — no npm dependency in class-ai-agent).

**Recommended:** `royalsolution-ontosight` (npm package pins PyPI `ontosight-codegraph` 0.2.0).
...
royalsolution-ontosight "<workspace-root>"

# Seed around a symbol (optionally scoped to a path)
# --symbol must exist in this project (confirm via codegraph_search first)
royalsolution-ontosight "<workspace-root>" --symbol <name> --path src/auth/

# Task-scoped subgraph (keyword FTS seeding)
royalsolution-ontosight "<workspace-root>" --task "auth flow" --hops 2 --max-nodes 200
```
...
3. royalsolution-ontosight "<workspace-root>" --symbol <name> --path <dir-from-search>
```
...
3. royalsolution-ontosight "<workspace-root>" --task "authentication flow" --hops 2 --max-nodes 200
```
...
6. royalsolution-ontosight "<workspace-root>" --symbol <name> --path <dir> --hops 3 --max-nodes 200
```

**Example** (replace paths and symbol with values from `codegraph_search` in **this** project):

```bash
royalsolution-ontosight "/path/to/workspace" --symbol deleteUser --path src/services/ --hops 3 --max-nodes 200
```
...
| Requirement | Notes |
|-------------|-------|
| Node 20+ | For `royalsolution-ontosight` |
