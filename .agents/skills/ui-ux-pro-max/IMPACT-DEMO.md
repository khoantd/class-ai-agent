...
Present this **before** running `royalsolution-ontosight` (with absolute workspace root as `[project-path]`):
...
```bash
royalsolution-ontosight "<workspace-root>" --symbol <name> --path <dir-from-search> --hops 3 --max-nodes 200
```
